...
export function PaymentsProvider({ children }: {children: ReactNode;}) {
  ...
  const createAndProcess = useCallback(
    async (contractId: string, amount: number) => {
      ...
      if (contract) {
        ...
        const notifMsg =
        result.status === 'paid' ?
        `Payment of LKR ${amount.toLocaleString()} completed successfully` :
        `Payment of LKR ${amount.toLocaleString()} failed: ${result.failureReason}`;
  ...
  const getPayment = useCallback(
    async (paymentId: string) => paymentsService.getPayment(paymentId),
    []
  );
  const getPaymentByContract = useCallback(
    async (contractId: string) =>
    paymentsService.getPaymentByContract(contractId),
    []
  );
  return (
