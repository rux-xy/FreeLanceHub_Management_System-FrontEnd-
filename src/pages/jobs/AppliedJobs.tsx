...
export function AppliedJobs() {
  ...
  // RESTORED: fetch real proposals from backend to show bid/status details
  ...
  useEffect(() => {
    const loadProposals = async () => {
      ...
      try {
        // listByFreelancer calls GET /proposals/my-proposals on the backend
  ...
  // RESTORED: merge job IDs from local applied list AND from real backend proposals
  // This keeps them in sync — backend is the source of truth, local list is a cache
  const proposalJobIds = freelancerProposals.map((p) => p.jobId);
  const allAppliedJobIds = [...new Set([...appliedJobs, ...proposalJobIds])];
  const appliedJobsList = jobs.filter((job) => allAppliedJobIds.includes(job.id));

  // RESTORED: helper to get the proposal object for a given job
    ...
    <Layout>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Jobs you've submitted proposals for</p>
        </div>
      ...
      {isLoading ? (
        ...
        // RESTORED: rich card view with proposal details per job
                ...
                <CardContent className="p-0">
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-5">
                      ...
                      {/* RESTORED: bid amount, estimated days, applied time */}
                          ...
                          <>
                            <span className="flex items-center gap-1">
                              ...
                              <span className="text-[#f97316]">
                                Your Bid: LKR {proposal.bidAmount.toLocaleString()}
                              </span>
                            </span>
                            <span className="flex items-center gap-1">
        ...
        <div className="text-center py-12 bg-[#111827]/30 rounded-xl border border-gray-800 border-dashed">
          ...
          <h3 className="text-xl font-medium text-gray-300">No applications yet</h3>
