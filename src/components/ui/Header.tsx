...
export function Header() {
  ...
  // Normalise role to uppercase so backend's 'ADMIN'/'CLIENT'/'FREELANCER'
  // all match correctly regardless of how they were stored.
  ...
  const NavLink = ({
    ...
    >
      {Icon && (
        <Icon
          className={`w-4 h-4 ${
            isActive(to) ? 'text-[#f97316]' : 'text-[#666666]'
          }`}
        />
      )}
      <span>{children}</span>
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-[#222222] bg-black/50 backdrop-blur-xl supports-[backdrop-filter]:bg-black/20">
          ...
          {user ? (
            <>
              ...
              >
                ...
                {chatCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#f97316] text-white text-[10px] font-bold flex items-center justify-center rounded-full shadow-sm">
                    {chatCount}
                  </span>
                )}
              </Link>
              ...
              <div className="hidden md:flex items-center space-x-4 ml-4 border-l border-[#222222] pl-4">
                <Link
                  to="/profile"
                  className="flex items-center space-x-2 text-sm font-medium text-[#888888] hover:text-white transition-colors"
                >
            ...
            {user ? (
              <>
                ...
                <div className="pt-4 border-t border-[#222222]">
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center space-x-2 text-sm font-medium text-red-400 mt-4 w-full"
                  >
